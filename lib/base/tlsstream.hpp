...
public:
	...
	auto get_executor() -> decltype(this->get_executor())
	{
		return this->get_executor();
	}

	template<class... Args>
	auto async_read_some(Args&&... args) -> decltype(this->async_read_some(std::forward<Args>(args)...))
	{
		return this->async_read_some(std::forward<Args>(args)...);
	}

	template<class... Args>
	auto async_write_some(Args&&... args) -> decltype(this->async_write_some(std::forward<Args>(args)...))
	{
		return this->async_write_some(std::forward<Args>(args)...);
	}
};
...
public:
	...
	// not allowed
	template<class... Args>
	auto async_write_some(Args&&... args) -> decltype(((AWS*)nullptr)->async_write_some(std::forward<Args>(args)...));
...
public:
	...
	auto async_read_some(boost::asio::mutable_buffer mb, boost::asio::yield_context yc) -> decltype(((ARS*)nullptr)->async_read_some(mb, yc))
	{
		...
		{
			auto count (m_Decompressor.read(ocobs, mb.data(), mb.size()));
